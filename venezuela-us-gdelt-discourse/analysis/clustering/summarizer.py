"""
Cluster summarizer using LLMs to generate human-readable descriptions.
"""
...
def format_samples_for_prompt(
    ...
    for i, text in enumerate(texts, 1):
        clipped = text[:max_chars_per_text] + "..." if len(text) > max_chars_per_text else text
        entry = f"{i}. {clipped}"
...
def create_summary_prompt(
    ...
    metadata_text = ""
    if metadata:
        if "top_group" in metadata:
            metadata_text += f"- Dominant source domain: {metadata['top_group']}\n"
        if "count" in metadata:
            metadata_text += f"- Total events in cluster: {metadata['count']:,}\n"
        if "sentiment_mean" in metadata:
            ...
            metadata_text += f"- Average model sentiment: {sentiment_label} ({sentiment:.2f})\n"
        if "gdelt_tone_mean" in metadata:
            metadata_text += f"- Average GDELT tone: {metadata['gdelt_tone_mean']:.2f}\n"
        if "gdelt_goldstein_mean" in metadata:
            metadata_text += f"- Average Goldstein scale: {metadata['gdelt_goldstein_mean']:.2f}\n"
        if "time_start" in metadata and "time_end" in metadata:
            metadata_text += f"- Time range: {metadata['time_start']} to {metadata['time_end']}\n"

    prompt = f"""Analyze these GDELT scraped article texts from Cluster #{cluster_id} about Venezuela-US discourse.
...
5. **Tone**: The overall emotional tone (e.g., critical, supportive, neutral, alarmed)
...
def summarize_cluster_with_openai(
    ...
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "You are an expert analyst of geopolitical media discourse."},
...
def summarize_all_clusters(
    ...
    for cluster_id in tqdm(clusters, desc="Summarizing"):
        ...
        metadata = {"count": len(cluster_df)}

        if "source_domain" in cluster_df.columns and len(cluster_df) > 0:
            counts = cluster_df["source_domain"].value_counts(dropna=True)
            metadata["top_group"] = counts.index[0] if len(counts) else None
        ...
        if "avg_tone" in cluster_df.columns:
            metadata["gdelt_tone_mean"] = cluster_df["avg_tone"].mean()
        if "goldstein_scale" in cluster_df.columns:
            metadata["gdelt_goldstein_mean"] = cluster_df["goldstein_scale"].mean()
