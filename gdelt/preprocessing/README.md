...
Generated preprocessing artifacts are written under `data/preprocessing/` by default. This folder keeps source code and hand-maintained configuration separate from runtime data products.
...
- Reuses existing IDs from `data/preprocessing/url_lookup.csv` when URL canonical forms already exist.
- Assigns new IDs only for unseen canonical URLs.
- Upserts `data/preprocessing/url_lookup.csv` (one row per canonical URL, with representative `Title`/`Text`/status).
...
What it does:
- Tokenizes `Text` in `data/preprocessing/url_lookup.csv` and writes JSON token arrays into `Tokens`.
...
- Writes/updates `data/preprocessing/url_filter_eval*.csv` with duplicate hash, cluster size, and duplicate decision.
- Writes duplicate-oriented summary counts to `data/preprocessing/url_filter_summary_counts*.csv`.
...
- Writes `data/preprocessing/text_relevance_tokens*.csv`.
...
What it does:
- Applies token relevance scores to each URL row in `data/preprocessing/url_lookup.csv`.
...
- Writes/updates `data/preprocessing/url_filter_eval*.csv` by `url_id` (existing IDs are updated with newly computed values).
- Rewrites `data/preprocessing/url_filter_summary_counts*.csv`.
- Writes stratified QA samples into `data/preprocessing/filter_samples*/`.
...
What it does:
- Reads `data/preprocessing/url_filter_eval*.csv`.
- Generates score histograms split by stage decisions.
- Writes `data/preprocessing/filter_stage_score_histograms*.png`.

## Output Files

- `data/preprocessing/url_lookup*.csv`: one row per canonical URL with scrape and scoring fields.
- `data/preprocessing/text_relevance_tokens*.csv`: token relevance table.
- `data/preprocessing/url_filter_eval*.csv`: row-level filter decisions and reason labels.
- `data/preprocessing/url_filter_summary_counts*.csv`: aggregate decision counts by stage.
- `data/preprocessing/filter_samples*/`: QA sample CSVs by stage/final decision.
- `data/preprocessing/filter_stage_score_histograms*.png`: visualization of score distributions by decision.
...
- If input is the default `..\data\gdelt_scraped.csv`, outputs use base names under `..\data\preprocessing\`:
  - `url_lookup.csv`, `text_relevance_tokens.csv`, `url_filter_eval.csv`, etc.
- If input is a different file, outputs in `..\data\preprocessing\` get a suffix from the input filename stem:
...
- The input scraped CSV is not overwritten by default; preprocessing artifacts are written under `data/preprocessing/` unless explicit output flags are provided.

## Stage Histogram Example

Default generated figure:
